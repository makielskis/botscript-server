 ...
 protected:
  ...
  void addTestUser() {
...

    {
      ...
      store.add_user(USERNAME, PW, EMAIL, [&actual_sid, &store](string sid, error_code e) {
        actual_sid = sid;
        ASSERT_FALSE(e);
    ...
    EXPECT_EQ(EMAIL, users[USERNAME]["email"].val());
    EXPECT_EQ(32u, actual_sid.length());
  }
...
TEST_F(UserStoreTest, AddUserTwiceTest) {
  {
    db_user_store store("test.kch", &io_service_);
    store.add_user(USERNAME, OTHER_PW, EMAIL, [](string sid, error_code e) {
      ASSERT_TRUE(e);
    });
...
TEST_F(UserStoreTest, SuccessfulLoginTest) {
  db_user_store store("test.kch", &io_service_);
  store.login(USERNAME, PW, [&store](string sid, error_code error) {
...
TEST_F(UserStoreTest, WrongPasswordLoginTest) {
  db_user_store store("test.kch", &io_service_);
  store.login(USERNAME, "wrong", [&store](string sid, error_code error) {
...
TEST_F(UserStoreTest, UserNotExistentLoginTest) {
  db_user_store store("test.kch", &io_service_);
  store.login("abc", PW, [&store](string sid, error_code error) {
    EXPECT_TRUE(store.user_sid().empty());
    EXPECT_TRUE(store.sid_user().empty());
    EXPECT_TRUE(store.sid_expire().empty());
    ASSERT_TRUE(error);
...
TEST_F(UserStoreTest, DoubleLoginNotPossibleTest) {
  db_user_store store("test.kch", &io_service_);
  store.login(USERNAME, PW, [&store](string sid1, error_code error) {
    ASSERT_FALSE(error);
    store.login(USERNAME, PW, [&store, sid1](string sid2, error_code error) {
      ASSERT_FALSE(error);
      EXPECT_EQ(1u, store.user_sid().size());
      EXPECT_EQ(1u, store.sid_user().size());
      EXPECT_EQ(1u, store.sid_expire().size());
      EXPECT_FALSE(sid1 == sid2);

      store.logout(sid1, [&store](error_code error) {
        EXPECT_TRUE(error);
      });

      store.logout(sid2, [&store](error_code error) {
        EXPECT_FALSE(error);
      });
    });
  });
...
TEST_F(UserStoreTest, SuccessfulLogoutTest) {
  db_user_store store("test.kch", &io_service_);
  store.login(USERNAME, PW, [&store](string sid, error_code error) {
    store.logout(sid, [&store, &sid](error_code error) {
...
TEST_F(UserStoreTest, WrongSessionLogoutTest) {
  db_user_store store("test.kch", &io_service_);
  store.login(USERNAME, PW, [&store](string sid, error_code error) {
    store.logout("wrongsession", [&store, &sid](error_code error) {
      EXPECT_EQ(1u, store.user_sid().size());
      EXPECT_EQ(1u, store.sid_user().size());
      EXPECT_EQ(1u, store.sid_expire().size());
      ASSERT_TRUE(error);
...
TEST_F(UserStoreTest, SuccessfulRemoveUserTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      ASSERT_FALSE(error);
      store.remove_user(sid, PW, [&store](error_code error) {
        EXPECT_FALSE(error);
        EXPECT_TRUE(store.user_sid().empty());
        EXPECT_TRUE(store.sid_user().empty());
        EXPECT_TRUE(store.sid_expire().empty());
...
TEST_F(UserStoreTest, WrongPasswordRemoveUserTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      ASSERT_FALSE(error);
      store.remove_user(sid, "wrong", [&store](error_code error) {
        EXPECT_TRUE(error);
        EXPECT_EQ(1u, store.user_sid().size());
        EXPECT_EQ(1u, store.sid_user().size());
        EXPECT_EQ(1u, store.sid_expire().size());
...
TEST_F(UserStoreTest, WrongSessionRemoveUserTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      ASSERT_FALSE(error);
      store.remove_user("wrongsession", PW, [&store](error_code error) {
        EXPECT_TRUE(error);
        EXPECT_EQ(1u, store.user_sid().size());
        EXPECT_EQ(1u, store.sid_user().size());
        EXPECT_EQ(1u, store.sid_expire().size());
...
TEST_F(UserStoreTest, SuccessfulSetPasswordTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      ASSERT_FALSE(error);
      store.set_password(sid, PW, OTHER_PW, [sid, &store](error_code error) {
        ASSERT_FALSE(error);

        store.logout(sid, [&store](error_code error) {
          ASSERT_FALSE(error);

          store.login(USERNAME, PW, [](string sid, error_code error) {
            EXPECT_TRUE(error);
          });

          store.login(USERNAME, OTHER_PW, [](string sid, error_code error) {
            EXPECT_FALSE(error);
          });
        });
      });
...
TEST_F(UserStoreTest, WrongOldPasswordSetPasswordTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      ASSERT_FALSE(error);
      store.set_password(sid, "wrong", OTHER_PW, [sid, &store](error_code error) {
        EXPECT_TRUE(error);
      });
    });
...
TEST_F(UserStoreTest, ChangeEmailTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      ...
      store.get_email(sid, [sid, &store](string email, error_code error) {
        ASSERT_FALSE(error);

        ASSERT_EQ(EMAIL, email);

        store.set_email(sid, PW, OTHER_EMAIL, [sid, &store](error_code error) {
          ASSERT_FALSE(error);
        });
      });
    });
...
TEST_F(UserStoreTest, BotManagementTest) {
  {
    ...
    store.login(USERNAME, PW, [&store](string sid, error_code error) {
      // Add bot1 -> BOT1
      store.add_bot(sid, BOT1, [sid, &store](error_code error) {
        ASSERT_FALSE(error);

        // Add bot2 -> BOT1,BOT2
        store.add_bot(sid, BOT2, [sid, &store](error_code error) {
          ASSERT_FALSE(error);

          // Add bot3 -> BOT1,BOT2,BOT3
          store.add_bot(sid, BOT3, [sid, &store](error_code error) {
            ASSERT_FALSE(error);

            // Add bot4 -> BOT1,BOT2,BOT3,BOT4
            store.add_bot(sid, BOT4, [sid, &store](error_code error) {
              ASSERT_FALSE(error);

              // Check if all bots are there
              store.get_bots(sid, [sid, &store](vector<string> bots, error_code error) {
                ASSERT_FALSE(error);

                EXPECT_EQ(4u, bots.size());
                ...
                store.remove_bot(sid, BOT4, [sid, &store](error_code error) {
                  ASSERT_FALSE(error);
                  ...
                  store.get_bots(sid, [sid, &store](vector<string> bots, error_code error) {
                    ...
                    store.remove_bot(sid, BOT2, [sid, &store](error_code error) {
                      ASSERT_FALSE(error);
                      ...
                      store.get_bots(sid, [sid, &store](vector<string> bots, error_code error) {
                        ...
                        // Remove bot1 -> BOT3
                        store.remove_bot(sid, BOT1, [sid, &store](error_code error) {
                          ASSERT_FALSE(error);
                          ...
                          store.get_bots(sid, [sid, &store](vector<string> bots, error_code error) {
                            ...
                            // Remove bot3 -> no bots
                            store.remove_bot(sid, BOT3, [sid, &store](error_code error) {
                              ASSERT_FALSE(error);
