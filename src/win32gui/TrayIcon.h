...
// You can use this class either by inheriting from it and overriding the
// OnMessage() method, or by instantiating this class directly and setting its
// listener object or function.
class CTrayIcon {
public:
  CTrayIcon(const char* name = "tray_icon", bool visible = false,
            HICON hIcon = NULL, bool destroy_icon_in_destructor = false);
  ...
  // The destructor may destroy the specified hIcon. If you want to avoid that,
  // call SetIcon(NULL, false) or SetDestroyIconInDestructor(false).
  ...
  bool ShowBalloonTooltip(const char* title, const char* msg,
                          ETooltipIcon icon = eTI_None);

  typedef void (*POnMessageFunc)(CTrayIcon* pTrayIcon, UINT uMsg);
  void SetListener(POnMessageFunc pOnMessageFunc) {
    m_pOnMessageFunc = pOnMessageFunc;
  }
...
private:
  ...
  static LRESULT CALLBACK MessageProcessorWndProc(HWND hWnd, UINT uMsg,
